...
bool RF24Mesh::beginWithDynamicID(uint8_t channel, rf24_datarate_e data_rate, uint32_t timeout){
  radio.begin();
  radio_channel = channel;
  radio.setChannel(radio_channel);
  radio.setDataRate(data_rate);  
  network.returnSysMsgs = 1;
  setNodeID(255);
  //Master wird nicht dynamisch vergeben
  
  mesh_address = MESH_DEFAULT_ADDRESS;
  if(!renewAddressDynamic(timeout)){
    return 0;
  }
...
uint16_t RF24Mesh::renewAddressDynamic(uint32_t timeout){
  ...
  while(!requestAddressDynamic(reqCounter)){
...
bool RF24Mesh::requestAddressDynamic(uint8_t level){    
  
#define MESH_MAXPOLLS 4
//Create Header with message Type NETWORK_POLL
...
for(uint8_t i=0; i<pollCount; i++){
  ...
  header.reserved = DEFAULT_NODE_ID;
...
if(type != NETWORK_ADDR_NODE_RESPONSE){
    return 0;
}
...
RF24NetworkHeader addrResponse;
memcpy(&addrResponse,network.frame_buffer, sizeof(addrResponse));
...
void RF24Mesh::DHCP(){
  ...
  printf("Incomming DHCP Request: "); printf(header.toString()); printf("NodeID: "); printf("%d", header.reserved); printf("\n");
  uint16_t newAddress;
    
  // Get the unique id of the requester
  uint8_t from_id = header.reserved;
  if(header.reserved == DEFAULT_NODE_ID){
    nodeIDRequest = TRUE;
  }
  if(!from_id){
  ...
  if( nodeIDRequest ){
    ...
    for(uint8_t nodeID = 1; nodeID < DEFAULT_NODE_ID; nodeID++){

      bool idIsInUse = FALSE;

      for(uint8_t k = 0; k <= addrListTop; k++){
        
        if(addrList[k].nodeID == nodeID || nodeID == DEFAULT_NODE_ID){
          printf("Is in use \n");
          idIsInUse = TRUE;
        }
      }
  ...
  for(int i=MESH_MAX_CHILDREN+extraChild; i> 0; i--){ // For each of the possible addresses (5 max)
    ...
    if(!found){
      printf("Free Address was found \n");
      header.type = NETWORK_ADDR_RESPONSE;
      if( nodeIDRequest ){
        header.type = NETWORK_ADDR_NODE_RESPONSE;
        header.reserved = freeNodeID;
      }
      header.to_node = header.from_node;
      printf("Header that is send to node: "); printf(header.toString()); printf(" New NodeID: %d\n", freeNodeID);
      ...
      if(nodeIDRequest){
        setAddress(freeNodeID,newAddress);
      }
      else{
